        ...
        <div className="flex-1 p-6 flex gap-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="w-[300px] flex-shrink-0">
              <div className="animate-pulse h-5 bg-gray-200 rounded w-24 mb-3" />
              <div className="space-y-2">
                {[1, 2, 3].map((j) => (
                  <div key={j} className="animate-pulse h-20 bg-gray-100 rounded-lg" />
                ))}
              </div>
            </div>
          ))}
  ...
  return (
    <div className="h-full flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <ProjectHeader project={project} activeView="board" />
        <TaskBoardView
